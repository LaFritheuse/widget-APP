...
const DayCell = ({ day, pnl, isToday, onPress }) => {
  ...
  return (
    <ScalePressable onPress={onPress} scaleTo={0.9} disabled={!hasTrade} style={styles.cellWrap}>
  ...
  return (
    <GlassCard delay={0}>
      ...
      <View style={styles.weekRow}>
        {DAY_LABELS.map((d) => <Text key={d} style={styles.weekLabel}>{d}</Text>)}
      </View>
...
const styles = StyleSheet.create({
  ...
  /* Case = toute la colonne (14.28% de large, aspectRatio:1 pour rester
     carrée), sans marge ni padding : les cases se touchent de tous les
     côtés comme une vraie mosaïque de calendrier, au lieu de petits carrés
     flottants avec des grands vides entre les lignes. */
  cellWrap: { flexBasis: '14.28%', aspectRatio: 1 },
